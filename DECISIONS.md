...
- **Контекст.** T053 (карточка T021, эпик E9, `deps: T052, T036`) — первый
  потребитель реестра T052: `dt resume` читает `sessions/<TASK_ID>.json` и
  выдаёт команды восстановления сессий после reboot. Дизайн §388: транскрипты
  Claude Code чистятся через `cleanupPeriodDays` (30 по умолчанию), поэтому
  `claude --resume <id>` против вычищенной сессии упадёт — `dt resume` **обязан**
  это учесть и деградировать, а не выдавать падающую команду. Развилка «как
  узнать, жив ли транскрипт» вынесена опросником Разработчику. Спека
  `specs/T053-resume/spec.md` (Analyzed).
- **Решение — детект по возрасту записи `last_seen`, транскрипты не трогаем.**
  Запись старше `RETENTION_DAYS = 30` считается потерявшей транскрипт → команда
  деградирует до `cd <wt> && claude` с подсказкой прочитать Handover; иначе
  `claude --resume <id>`. Порог совпадает с дефолтом `cleanupPeriodDays`.
  Непарсящийся или naive `last_seen` трактуется как свежий (best-effort: лучше
  предложить резюм, чем спрятать сессию).
- **Отвергнутая альтернатива A — `stat` файла транскрипта.** Точнее по факту, но
  требует вычислять недокументированный путь
  `~/.claude/projects/<munged-cwd>/<session_id>.jsonl` (схема munging cwd не
  специфицирована, ломается при смене версии Claude Code) и прямо конфликтует с
  acceptance §434/#8 «ни один компонент не читает внутренние файлы
  транскриптов». Тот же принцип уже зафиксирован в T052 (§358: опираемся только
  на документированные поля хука). Привязка к приватной раскладке — цена, не
  окупающая точности.
- **Отвергнутая альтернатива B — всегда `--resume`.** Проще всего, но нарушает
  §388: после ретенции команда упадёт, а `dt resume` для того и существует,
  чтобы этого не случилось.
- **Следствия.** Детект приблизителен (пользователь мог переопределить
  `cleanupPeriodDays`, или транскрипт удалён вручную раньше) — сознательный
  размен точности на полную развязку от приватного хранилища Claude Code.
  Реестр остаётся **удобством, не источником истины** (§437–439): актуальный
  Handover (T055) обязателен независимо. Точное чтение `cleanupPeriodDays` из
  настроек CC — при потребности отдельной задачей.

### 2026-08-01 — T052: реестр сессий — файл-на-задачу, атомарная запись, хук читает stdin

- **Контекст.** T052 (карточка T020, эпик E9, `deps: T051`) замыкает
  автоматическую привязку сессии к задаче. T051 дал `dt context --hook`
  (ориентацию), но связь «живая сессия ↔ задача ↔ каталог» нигде не
  фиксировалась. Наблюдение §362: SessionStart-хук получает `session_id` и
  `cwd` — реестр строится сам. Контракт хука сверен по официальной
  документации Claude Code **до** реализации (§441): поля stdin
  (`session_id`/`cwd`/`source`, без `transcript_path`), невозможность заблокировать
  старт (exit 2/любой ненулевой лишь показывает stderr), форма `matcher`. Спека
  `specs/T052-session-registry/spec.md` (Analyzed); открытых развилок нет —
  всё пиновано дизайн-документом и конвенциями репо.
- **Решение 1 — файл-на-задачу, а не общий JSON (§370).**
  `sessions/<TASK_ID>.json` с `{session_id, cwd, last_seen}`. Ключ — в имени
  файла (`<TASK_ID>`), в JSON не дублируется. Параллельные хуки разных задач
  пишут в разные файлы → блокировки не нужны, гонок на read-modify-write нет.
  Альтернатива (общий реестр) отвергнута: потребовала бы lock и всё равно ловила
  бы гонки.
- **Решение 2 — атомарная запись temp + `Path.replace`.** Запись во временный
  файл в том же каталоге и атомарный rename: параллельный `dt resume` (T053)
  прочитает старый или новый файл, но никогда полузаписанный. Перезапись целиком
  — «последняя запись побеждает» (один worktree = одна задача, `last_seen`
  просто растёт).
- **Решение 3 — чистое ядро `dt/sessions.py` (typer-/git-free), но с FS.**
  Инвариант «чистого ядра» в репо — свобода от `typer` и `git`, не от
  файловой системы (образец — `tasks.py` пишет записи задач). Сериализация +
  атомарная запись/толерантное чтение живут в ядре; обёртка `context_cli.py`
  добывает stdin/`session_id`/`cwd`/время. `task_id` валидируется по
  `^T[0-9]{3,}$` до выхода на ФС — та же защита от path traversal, что в
  store-слое (`tasks._ID_RE`): write → `ValueError`, read → `None` (находка
  qodo, важна для будущего `dt resume T<NNN>` с пользовательским ID).
- **Решение 4 — запись реестра как побочный эффект после payload.** В
  `_emit_hook` сначала печатается `hookSpecificOutput`, потом best-effort запись
  реестра под `contextlib.suppress(Exception)`: провал записи (нет прав, диск)
  не должен стоить уже собранного контекста. Запись — только для привязанной
  сессии с непустым `session_id` (у непривязанной нет `<TASK_ID>`-ключа).
  `isatty`-гард в `_read_hook_input` — чтобы ручной `dt context --hook` в
  терминале не завис на `read()`.
- **Решение 5 — `last_seen` tz-aware.** Дизайн-пример был naive-local
  (`2026-07-29T14:12:03`), но `flake8-datetimez` (DTZ) в репо запрещает naive
  datetime → tz-aware ISO-8601 с offset (образец `tasks._today()`). Значение
  переносится между машинами (resume) — offset однозначен.
- **Решение 6 — шаблонный хук без `matcher`.**
  `template/.claude/settings.json` c единственным блоком `SessionStart` →
  `dt context --hook`. `matcher` опущен ⇒ срабатывание на всех `source`
  (`startup`/`resume`/`clear`/`compact`/`fork`, §353). Не догфудим хук в
  собственный `.claude/` пакета: store dreamteam пуст до `dt migrate tasks`
  (T042).
- **Границы (не в T052).** `dt resume` (чтение реестра, `--tmux`) — T053;
  statusline — T054; Handover как живая секция + PreCompact-хук + детект
  устаревания — T055; секция методики `sessions` — T047.
- **Отвергнутые альтернативы.** (1) Общий `sessions.json` — гонки/lock
  (см. Решение 1). (2) Различать поведение хука по `source` (Handover только на
  `clear`/`resume`) — отложено в T055; здесь хук единообразен. (3) Naive-local
  `last_seen` из дизайн-примера — нарушает DTZ (см. Решение 5).

### 2026-08-01 — T051: `dt context` — ориентация сессии, резолв по ветке, hook никогда не блокирует

- **Контекст.** T051 (карточка T019, эпик E9, `deps: T034, T036`) решает потерю
  ориентации после reboot/`/clear` (§291): нет ключа «сессия ↔ задача»,
  переживающего смерть процесса. Ключ — ветка/worktree (§299): имя ветки уже
  кодирует ID. Спека `specs/T051-context/spec.md` (Analyzed); 2 развилки
  согласованы опросником. **Порядок:** взял T051 вместо T042 — по §6.4 T042
  («точка невозврата») берётся после T051–T053; развилку явно согласовал.
- **Решение 1 — расслоение как T035/T037/T040.** Чистое ядро `dt/context.py`
  (typer-free И git-free): `resolve_task_id`, `build_context` (над
  предвычисленными git-фактами), рендеры human/json/hook. Обёртка
  `context_cli.py` добывает git-факты (`git_context`, `list_worktrees`,
  `main_worktree`, текст BACKLOG, `current-task`) и подаёт в ядро.
- **Решение 2 — единый порядок резолва (§309).** `DT_TASK` → префикс ветки
  `T<NNN>` → `by-worktree/<slug>/current-task` → непривязанная (легальный
  режим). Разрешённый, но отсутствующий в store ID → непривязанность + пометка
  «висячий», не ошибка.
- **Решение 3 — расхождение BACKLOG только при управляемом блоке (опросник,
  развилка A).** Сравнение store с BACKLOG.md **основной копии**
  (`main_worktree`, новый публичный хелпер в `paths.py`) через
  `backlog_divergence` (T040) — но лишь если `has_managed_block` (новый в
  `backlog.py`) и дельта ненулевая. Иначе на неадаптированном проекте (сам
  dreamteam до T042) было бы ложное «+всё».
- **Решение 4 — блокеры = `deps` ≠ `done` (опросник, развилка B).** Показываем
  незакрытые зависимости (что мешает вести задачу); согласуется с `dt task
  ready`. Обратные `blocks` не показываем — шум.
- **Решение 5 — `--hook` никогда не блокирует старт (§356).** Весь сбор в
  `try/except Exception` → любая ошибка = exit 0 и пустой вывод; успех →
  `{hookSpecificOutput:{hookEventName:"SessionStart",additionalContext}}`,
  `additionalContext` обрезан до 2000 симв. `BLE001` в глобальном ignore —
  широкий except здесь дизайн-обязателен, без noqa.
- **Решение 6 — запись `context.line`, не `current-task`.** `dt context`
  обновляет только строку statusline под slug cwd (§1068); привязку
  (`current-task`) пишет `dt task start` (T039). Транскрипты не читаем (§358).
- **Границы (не в T051).** SessionStart-хук в `settings.json` + реестр
  `sessions/<id>.json` — T052; statusline-скрипт — T054; детект устаревания
  Handover — T055; `dt run` enforcement каталога — T056 (T051 даёт лишь
  строку-детект расхождения каталога).
- **Рассмотренные альтернативы.** (1) Ключ = session-id/имя сессии — отвергнут
  (§299/§318: не переживает процесс / агент не может переименовать сессию);
  ветка уже кодирует задачу. (2) Показывать расхождение всегда — отвергнуто
  (ложный шум, развилка A). (3) Дублировать резолв `_main_worktree`/чтение
  BACKLOG в ядре — отвергнуто (git-факты подаёт обёртка).

### 2026-08-01 — T041: `dt state export/import` — JSON-bundle, pre-check конфликтов, stdio-pipe

- **Контекст.** T041 — девятая задача E1 (карточка T009, `deps: T034`).
  Оперативный слой состояния сознательно не ездит с git (design §233); нужен
  явный ручной канал переноса между машинами Разработчика. Переносятся только
  записи задач и счётчик (§209); `sessions/` и `by-worktree/` машинно-
  специфичны. Спека `specs/T041-state-transfer/spec.md` (Analyzed); 2 развилки
  согласованы опросником.
- **Решение 1 — расслоение как T035/T037.** Чистое ядро `dt/state.py`
  (typer-free И git-free): `export_bundle`, `serialize`/`parse`,
  `import_bundle`. Обёртка `state_cli.py` резолвит store и делает file/stdio
  I/O. git не участвует вовсе.
- **Решение 2 — формат JSON одним файлом (опросник, развилка A).**
  `{dt_state_version, counter, tasks:[...]}`; запись = `model_dump` + `body`,
  unknown-поля сохраняются через `extra='allow'`. Согласуется с `--json`,
  human-readable diff, лёгкая детекция конфликтов. Отвергнут tar.gz
  (непрозрачный бинарь). `dt_state_version` проверяется: файл новее
  поддерживаемого — отказ, а не тихое неверное чтение.
- **Решение 3 — поддержка `-` (stdin/stdout) (опросник, развилка B).**
  `dt state export - | ssh other 'dt state import -'` — прямой канал без
  промежуточного файла, ровно под сценарий. Инфо-строки идут в stderr, stdout
  чист под bundle.
- **Решение 4 — pre-check конфликтов до записи (§211).** `import` сначала
  валидирует все ID (см. Решение 6), ловит дубли, считает `conflicts =
  bundle ∩ local`. Без `--on-conflict` и при конфликте → `TaskError` со
  **списком всех** ID, **ни одной** записи. `skip` — оставить локальные,
  `overwrite` — заменить; неконфликтные всегда пишутся. Все ошибки — до записей
  (свойство «всё-или-ничего» на этапе решения).
- **Решение 5 — счётчик = max, никогда не вниз.** После импорта
  `advance_counter(max(bundle.counter, наибольший импортируемый ID-номер))`;
  сам `advance_counter` берёт max с локальным. Гарантия: `allocate_id`
  (T034) впредь не переиспользует номер. Поднимаю всегда (даже skip-only).
- **Решение 6 — тонкий публичный API в `tasks.py`.** Введены `read_counter`,
  `advance_counter` (max-семантика), `record_path` (валидирует `T[0-9]{3,}` →
  защита от path traversal из подделанного bundle). Делегируют в существующие
  приватные; переиспользуются в T042 (migrate). Не дублируем layout store.
- **Не в scope.** Валидация графа при импорте (циклы/висячие ссылки) —
  `dt task check` (T035) отдельно; перенос `sessions/`/`by-worktree/` —
  сознательно исключён (§209); шифрование/дельта-синхронизация/слияние тел —
  вне задачи. Атомарность транзакции не требуется (ручной низкочастотный
  канал), достаточно pre-check до записей.
- **Рассмотренные альтернативы.** (1) tar.gz — отвергнут (Решение 2).
  (2) Path-only без `-` — отвергнут в пользу прямого ssh-pipe (Решение 3).
  (3) Дублировать чтение счётчика/путь в `state.py` вместо публичного API
  `tasks.py` — отвергнуто (layout store — одна точка правды).

### 2026-08-01 — T040: `dt backlog sync` — управляемый блок, guard основной ветки, ранняя функция расхождения

- **Контекст.** T040 — восьмая задача E1 (карточка T008, `deps: T034`), точка
  входа. Под новым слоем состояния источник истины о задачах — оперативный
  store; `BACKLOG.md` становится его **проекцией** в долговременный git-слой
  («что собираемся делать»), статус-независимой (design §215–216). Спека —
  `specs/T040-backlog-sync/spec.md` (Analyzed); 3 развилки согласованы
  опросником.
- **Решение 1 — расслоение как в T035/T037.** Чистое ядро `dt/backlog.py`
  (typer-free И git-free): `backlog_items` (проекция store: незавершённые,
  сортировка по **числовому** ID), `render_item`/`render_block`,
  ...
  обёртка `backlog_cli.py` из `paths.py` и подаёт в ядро.
- **Решение 2 — управляемый блок между маркерами (опросник, развилка A).**
  `sync` правит только регион `<!-- dt:backlog:begin -->` … `end`, сохраняя
  вводную прозу (правила формата, scope-guard, роадмап). Маркеров нет →
  дописывает блок в конец (self-bootstrap). Замена — regex с **функцией**-
  заменой, а не строкой: обратные слэши/`\g` в заголовках не интерпретируются.
  Отвергнута полная регенерация файла: снесла бы ручной роадмап/эпики/историю
  (у самого dreamteam критично). Идемпотентно, парсибельно для T051.
- **Решение 3 — guard основной ветки + `--force` (design §216).** `sync` на
  ветке ≠ локальной default-ветки (`default_base_branch`, T036, офлайн) →
  отказ exit 1 с подсказкой; detached HEAD и «вне git» — тоже отказ. Иначе две
  ветки, каждая переписывающая BACKLOG.md, дают ровно тот merge-конфликт, от
  которого уходим. `--force` — аварийный люк.
- **Решение 4 — функция расхождения строится сейчас (опросник, развилка B).**
  `backlog_divergence(store, backlog_text) → (added, removed)` реализована в
  T040 с тестами, хотя её единственный потребитель `dt context` (T051) ещё не
  готов — следуем декомпозиции дизайн-карточки T008. CLI-команды у неё в T040
  нет (нечему потреблять): T040 отдаёт готовую чистую функцию, резолв пути
  «основной рабочей копии» (design §880) вшивает T051.
- **Формат строки (развилка C).** `- **T040** — [<created>] <title> (deps: …;
  spec: …)`; дата/deps/spec опускаются, если пусты. Совпадает с текущим
  шаблонным `[<date>]`.
- **Догфудинг отложен.** На самом dreamteam store пуст до `dt migrate tasks`
  (T042) — `sync` на его BACKLOG.md не гоняем (снёс бы ручной роадмап). Все
  тесты — на temp-store / реальном temp-git-репо (fixture образца T039).
- **Рассмотренные альтернативы.** (1) Полная регенерация файла — отвергнута
  (см. Решение 2). (2) Отложить `backlog_divergence` до T051 — отвергнута в
  пользу буквы дизайна (Решение 4). (3) Тонкая вставка блока под `## Items`
  вместо конца файла — возможный follow-up, не MVP (детерминированность
  важнее косметики).

### 2026-08-01 — T039: `dt task start` — транслит-slug, привязка нового worktree, tmux внутри CLI

- **Контекст.** T039 — композитный старт задачи (карточка T007, `deps: T034,
  T036`), точка входа E1. Сворачивает пять ручных шагов (статус, имя ветки,
  worktree, привязка, окно tmux) в одну команду без флагов от человека
  (design §326/§336, §858–864). Спека — `specs/T039-task-start/spec.md`
  (Analyzed); 4 развилки согласованы опросником.
- **Решение 1 — расслоение как в T035/T036.** Чистые ядра: `dt/slug.py`
  (транслит + `slugify` + `branch_name`), `dt/starts.py` (`plan_start`
  decision-table, `context_line`, `extract_handover`, `write_binding`) —
  typer-free И git-free. Git-эффекты (`local_branch_exists`, `add_worktree`)
  в `paths.py`; мутация записи (`start_task`) в `tasks.py`; оркестрация —
  команда `start` в `task_cli.py`. `plan_start(branch, path, *,
  worktree_exists, branch_exists)` покрывает все 4 комбинации без git.
- **Решение 2 — slug транслитерацией ru→lat (опросник).** Заголовок задачи
  часто кириллический; ветка/окно tmux/URL должны быть ASCII. Простая таблица
  (щ→shch, ж→zh, ё→e, ъ/ь→''), lowercase, не-`[a-z0-9]`→`-`, обрезка по словам
  до 40 симв. Приближение, не ГОСТ: нужна лишь читаемость, уникальность даёт
  префикс `T<NNN>`, коллизии slug безвредны. Отвергнуты «unicode как есть»
  (кривые ветки/окна) и «без slug» (теряется человекочитаемость).
- **Решение 3 — привязка под slug НОВОГО worktree (опросник).**
  `current-task`/`context.line` пишутся под `worktree_slug(новый_путь)` —
  там пойдёт работа, statusline/`dt context` (T051/T054) сразу наполнены;
  вызывающая (основная) копия не привязывается к задаче. Резолв в новом
  worktree и так идёт по имени ветки `T<NNN>` — `current-task` там
  подстраховка, но `context.line` под тем же slug нужен statusline сразу,
  поэтому пишем оба единым действием.
- **Решение 4 — tmux внутри CLI, best-effort (опросник).** `dt task start`
  сам зовёт `tmux rename-window -t "$TMUX_PANE" <id>` при заданном `$TMUX`;
  вне tmux/без бинаря/при ошибке — тихий пропуск, `rename_window` **никогда**
  не бросает и не влияет на exit-код (§419). Это subprocess внутри dreamteam,
  отдельного allow-листа Claude не требует; allow-лист `tmux rename-window`
  в `.claude/settings.json` — задача E3.1 (v0.4), не блокирует T039.
- **Решение 5 — база ветки от локального main без fetch (опросник).**
  `git worktree add -b <branch> <path> <base>`, `<base>` =
  `default_base_branch()` (T036, локальный `origin/HEAD`→`main`). Composite-
  команда офлайн и быстрая; «от свежего main» обеспечивается **дисциплиной**
  (агент фетчит и ребейзит отдельно перед PR), а не сетевым вызовом внутри
  `start`. Отвергнуты «текущий HEAD» (легко уйти от чужой ветки/детача) и
  «main + авто-fetch» (сеть в composite-команде, ломается офлайн).
- **Порядок эффектов и идемпотентность.** Сначала git (`worktree add`) — самый
  склонный к отказу шаг; при провале запись/привязка не трогаются (нет
  полу-применённого состояния, A6). `resolve_path`→`(path, exists)`:
  существующий worktree переиспользуется, ветка из поля `branch` записи не
  перегенерируется — повторный `start` не падает и не дублирует.
- **Статус из любого состояния.** `start` переводит в `doing` даже из
  `done`/`dropped` (переоткрытие легитимно); гейта нет — при необходимости
  тонкий guard заведём отдельным T-ID.
