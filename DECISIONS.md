...
### 2026-08-01 — T058: `dt task check` предупреждает о дрейфе frontmatter id ↔ имя файла

- **Контекст.** Follow-up к находке ревью T038 (qodo): имя файла — канонический
  ID (арбитр `O_EXCL`, T034), и T038 канонизировал `load_all_tasks`
  (`record.id = path.stem`), чтобы `find`/`board`/`ready` не отдавали
  неоткрываемый ID. Но канонизация в памяти **скрывает** рассогласование от
  проверок. T058 добавляет его обнаружение (заведён отдельным T-ID по просьбе
  Владимира и выполнен сразу).
- **Решение — WARNING, детект в едином проходе `_scan_records`, не ERROR.**
  Записи парсятся **один раз**: `_scan_records(store)` возвращает и
  канонизированный dict, и пары дрейфа `(stem, raw_id)` — собранные **до**
  переписывания `id` на stem. `load_all_tasks` стала тонкой обёрткой
  (`_scan_records(store)[0]`), `check_tasks` берёт из одного прохода и dict, и
  drift; `_id_mismatch_issues(drift)` строит **WARNING** по парам. Не ERROR:
  стор самоисцеляется (инструмент работает через канонизацию), валить
  pre-push/CI из-за косметического рассогласования нельзя — но пользователю
  сообщаем, чтобы поправил файл.
  - *Почему в проходе загрузки, а не отдельным чтением:* `check_tasks` работал
    поверх канонизированного dict, где `id` уже равен stem — дрейф там невидим.
    Отдельный второй файловый проход (первая версия) удваивал парсинг
    (замечание ревью qodo на T058-PR) — заменён на сбор дрейфа в том же
    единственном проходе.
  - *Альтернатива ERROR* отвергнута: противоречит самоисцелению T038 и
    ломала бы гейт на ровном месте при ручной правке.
