...
- **T026** — [2026-07-05] Роли команды: Архитектор (субагент) +
  Дизайнер (Claude Design MCP) в шаблоне.

  **Контекст.** Предложение из обсуждения с Ионой: каждый derived-
  проект получает переиспользуемый контур сотрудничества — лид
  (сессия Claude Code) + read-only субагент-Архитектор
  (`.claude/agents/architect.md`, консультант по решениям, предлагает
  ADR) + Дизайнер (внешний Claude Design через user-scope MCP).
  Источник истины ролей — файлы методики проекта, не внешняя память.
  Обе роли ставятся по умолчанию; выбор использования — runtime.

  **Крупная фича — спека готова:** `specs/T026-team-roles/spec.md`
  (draft Ионы по README + analyze-проход Гвидо со сверкой по коду;
  §5–§6 переписаны под реальную подложку: `partials/` под `_exclude`
  + Jinja-сборщик со `strip_frontmatter`, гарантия строки-импорта
  через пост-апдейт-хук в `cli.py`, а не copier `_migrations`).

  **Сердце фичи (§5–§6 спеки):**
  - **Рендер шапка+тело.** Функциональный frontmatter субагента
    (`name/tools/model/description`) обязан дожить до рендера, но
    i18n-конвейер (`_tasks_post_render.py`) срезает ведущий
    frontmatter. Решение — тело-партиалы в `partials/**` (в
    `_exclude`: видны для `{% include %}`, не эмитятся), шапка из
    copier-переменных, склейка Jinja-сборщиком `.claude/agents/
    architect.md` со `strip_frontmatter`-фильтром (`extensions/
    frontmatter.py`).
  - **Авто-пикап по `dreamteam update`.** Всё новое — новыми
    файлами (чистый three-way merge); Архитектор заводится нулевым
    касанием (авто-дискавери агентов); методика ролей —
    `.claude/team-roles.md` + одна строка `@import` в `CLAUDE.md`,
    доставку которой гарантирует идемпотентный пост-апдейт-хук в
    `cli.py`.

  **Открытые вопросы (перед Implement, см. хвост спеки):** носитель
  `description` в партиале; реальность и эндпоинт Claude Design MCP
  (§W2 — до зашивания команды в методику); попадание `extensions/` +
  `partials/` в `.bundle`.

  **Acceptance:** см. §6 приёмка + §8 фазы/тесты спеки. Кратко:
  на каждом языке `.claude/agents/architect.md` — ровно один
  frontmatter, `model == architect_model`, тело на языке методики
  без остатков переводческого frontmatter; `partials/`/`extensions/`
  не утекли в вывод; `update` на старом проекте — пикап без
  конфликтов, строка-импорт есть даже при переписанном `CLAUDE.md`,
  повторный update идемпотентен; `translate_check.py` (со сканом
  `partials/`) зелёный; 4 pre-push проверки зелёные.

  **Тип:** новая фича методики (template + i18n + `cli.py` + tests),
  через PR в dreamteam-репо. Версионирование — MINOR. Одна фича
  (Архитектор + Дизайнер вместе, решение Разработчика), фазы §8 —
  отдельные коммиты.
