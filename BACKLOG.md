...
- **T060** — [2026-09-04] Связь между сессиями в соседних worktree: правило
  «изоляция ≠ молчание» в шаблонный `CLAUDE.md`. Сейчас раздел
  «Параллельная работа в нескольких git worktree» описывает только изоляцию
  (чего с чужой веткой НЕ делать) и реестр `git worktree list`; про то, что
  соседней сессии положено **писать**, не сказано нигде — и молчание выглядит
  правильным поведением. Обкатано в `calque` 2026-09-04 и уже записано в
  личный глобальный `CLAUDE.md` Разработчика, в шаблон едет проверенным.
  Три триггера обязательного сообщения соседу (`ListAgents` + `SendMessage`):
  (1) темы смежные — спросить ДО написания кода, что уже написано и какие
  общие файлы/константы сосед трогает; (2) развилка похожа на чужую —
  спросить соседа, не решена ли она уже, ПРЕЖДЕ чем выносить выбор человеку;
  (3) общий внешний ресурс (сборочная ВМ, сервер, хранилище) — спросить ДО
  команды, ответ дождаться, а не предположить. Ограничители, без них правило
  станет шумом: одно сообщение с конкретными вопросами, а не переписка; и
  ответ соседа НЕ заменяет решения человека — сосед сообщает факты о своём
  коде, а согласие на слияние, отступление от конвенции и выбор из развилки
  даёт только человек. Повод: две сессии независимо шли к чтению одного
  модуля, а одну и ту же развилку человек за утро решил дважды, не заметив
  повтора. По дороге проверить, не даёт ли реестр сессий (T052) более точный
  способ узнать соседей, чем `ListAgents`. deps: —. Acceptance: блок в
  разделе «Параллельная работа в нескольких git worktree» шаблонного
  `CLAUDE.md`, между «Изоляция (жёстко)» и «Жизненный цикл worktree», на всех
  языках `i18n/*` (`scripts/translate_check.py` зелёный); текст несёт три
  триггера, оба ограничителя и повод; запись в `CHANGELOG.md`.
