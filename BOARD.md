...
- **T030** — [closed 2026-07-30, PR T030-worktree-methodology] Методика
  ...
  живёт знание о проекте (memory-agnostic)» и «Параллельная работа в
  нескольких git worktree» (реестр / ритуал старта / изоляция / жизненный
  цикл / уборка-с-запросом / общие dev-сервисы / память ≠ общая между
  папками). Source-of-truth `ru` + re-bootstrap `en/fr/de/zh`
  (`source_hash` сверен, translate_check 44 ok). Нейтральные термины, без
  ADR (структура генерируемых файлов не менялась).
