...
2. `PROJECT.md` — текущее состояние проекта.
...
4. **После фиксации `CONCEPT.md` не редактируется.** Текущее
   состояние ведётся в `PROJECT.md`. Если концепция кардинально
   меняется (rare, pivot) — добавляется `concepts/v2-...md`,
   `v3-...md` (ADR-pattern, но для концепций).

Заготовка лежит в `CONCEPT.template.md` (копируется в `CONCEPT.md`
и заполняется).
